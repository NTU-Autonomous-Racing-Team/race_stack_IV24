...
class GapFinderAlgorithm:
    ...
    def generate_safety_bubble(self):
        ...
        for i in range(
            max(0, self.min_range_index - radius_count),
            min(self.min_range_index + radius_count + 1, len(self.ranges)),
        ):
            self.ranges[i] = 0.0
    ...
    def find_max_gap(self):
        ...
        if self.min_range_index < len(self.ranges) // 2:
            ...
            self.max_gap_index = self.ranges.index(
                max(self.ranges[self.min_range_index :])
            )
        else:
    ...
    def find_twist(self):
        ...
        init_angZ = self.scan_angle_increment * (
            self.max_gap_index - len(self.ranges) // 2
        )
...
class GapFinderNode(Node):
    def __init__(self, hz=50):
        ...
        self.drive_publisher = self.create_publisher(
            AckermannDriveStamped, "/nav/drive", 10
        )

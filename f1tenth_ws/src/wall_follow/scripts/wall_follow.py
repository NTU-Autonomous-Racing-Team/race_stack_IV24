...
class WallFollow(Node):
    ...
    def __init__(self):
        ...
        self.pub_drive = self.create_publisher(AckermannDriveStamped, "nav/drive", 10)
    ...
    def scan_callback(self, scan_data):
        ...
        alpha = -1 * math.atan2(
            (distance_a * math.cos(theta) - distance_b), (distance_a * math.sin(theta))
        )
        ...
        if dt != 0:
            ...
            # else:
            self.drive_msg.drive.speed = -1 * float(
                min(0.5 * (1 / 1.2) ** (steering_angle_degrees - 15), 10)
            )

            self.pub_drive.publish(self.drive_msg)
            self.get_logger().info(
                f"steering_angle: {steering_angle_degrees:.2f} | speed: {self.longitudinal_vel:.2f} | {actual_distance:.2f} | {lookahead_distance:.2f}"
            )

...
class PurePursuit(Node):
    ...
    def __init__(self):
        ...
        with open(waypoint_file_name, newline="") as f_in:
            ...
            for row in reader:
                self.waypoints = np.vstack(
                    [self.waypoints, [float(row[0]), float(row[1]), float(row[2])]]
                )
        ...
        # self.sub_output_select = self.create_subscription(
        #     String, "output_select", self.output_select_callback, 1
        # )
    ...
    def output_select_callback(self, output_select_data):
        self.output_select_data = output_select_data.data

    def pose_callback(self, odom_data):

        if True:  # self.output_select_data == "global_planner":
            ...
            car_heading = np.array(
                [math.cos(car_heading_angle), math.sin(car_heading_angle)]
            )
            ...
            for waypoint_index in range(len(self.waypoints)):
                ...
                goal_point_speed = waypoint[2]

                # Calculate the distance from the car to the waypoint
                waypoint_distance = np.sqrt(
                    (waypoint[0] - odom_data.pose.pose.position.x) ** 2
                    + (waypoint[1] - odom_data.pose.pose.position.y) ** 2
                )

                # Calculate the vector from the car to the waypoint
                car_to_waypoint = np.array(
                    [
                        waypoint[0] - odom_data.pose.pose.position.x,
                        waypoint[1] - odom_data.pose.pose.position.y,
                    ]
                )
            ...
            rotated_goal_point = vehicle_quaternion.rotate(
                [self.goal_point[0], self.goal_point[1], 0.0]
            )
            goal_point_vehicle_frame = [
